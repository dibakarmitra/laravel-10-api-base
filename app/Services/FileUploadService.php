...
class FileUploadService
{
     ...
     */
    const UPLOAD_PATH = 'uploads/{folder}/{id}/{type}/{filename}';
    ...
    {
        if (!$file->isValid()) {
            throw new InvalidFileException($file->getErrorMessage());
        }

        if ($file->getSize() === false) {
            throw new InvalidFileException('File failed to load.');
        }
        ...
        $filePath = self::getUploadDir($filenameToStore, $type, (string) $folder, $id, $size,  $disk);
    
        // Upload File to storage disk
        $fileContent = fopen($file, 'r+');
        if (!$fileContent) {
            throw new InvalidUploadException('Could not read file from disk...');
        }
        ...
        if (!$path) {
            throw new InvalidUploadException('The file could not be written to disk...');
        }
     ...
     * @throws InvalidFileException
     */
    public static function uploadFromUrl(string $url, array $options = []): array
    {
        ...
        $filePath = trim(parse_url($url)['path'], '/') ?? null;
        // get file extension
         ...
         */
        // get filename with extension
        $filenameWithExtension = mb_strcut(pathinfo($filePath, PATHINFO_FILENAME), 0, 255 - ($extension ? strlen($extension) + 1 : 0), mb_detect_encoding($filePath)). '_' . time() . ($extension ? '.' . $extension : '');
        // $filenameWithExtension = $file->hashName() ?: $file->getClientOriginalName();
        ...
        // Make a file path where image will be stored [uploads/{folder}/{type}/{filename}.{ext}]
        $filePath = self::getUploadDir($filenameToStore, $type, (string) $folder, $id);
        
        $path = Storage::disk($disk)->put($filePath, $file, $args['visibility']); // very nice for very big files
    ...
    {
        $defaults = self::getConfig();
        $disk = $disk ?? $defaults['disk'] ?? self::$disk;
        
        return $disk;
    }

    /**
     * Get FileUploadService configurations
     *
     * @return array<string, string>
     */
    protected static function getConfig(): array
    {
        return ['visibility' => self::VISIBILITY_PUBLIC, 'disk' => self::$disk];
    }
    ...
    public static function getFile(string $path, string $disk = null): ?string
    {
        if (!Storage::disk(self::getDisk($disk))->exists($path)) {
            throw new MissingFileException("File $path does not exists.");
        }
     ...
     */
    private static function getUploadDir(string $filename, string $type = self::FILE, string $folder = null, string $id = null, string $size = null, string $disk = null): string
    {
        if($disk === 's3') {
            $dir = '{folder}/{id}/{size}/{filename}';

            return str_replace('//', '/', trim(strtr($dir, [
                '{folder}' => $folder,
                '{id}' => $id,
                '{size}' => $size,
                '{filename}' => $filename,
